...
def LocalRuleSet.add (rs : LocalRuleSet) :
  ...
  | .global (.normSimpRule r) =>
    ...
    let simpTheoremsArrayNonempty : 0 < simpTheoremsArray.size := by
      simp [simpTheoremsArray, Array.size_modify, rs.simpTheoremsArrayNonempty, -List.length_pos]
...
def LocalRuleSet.erase (rs : LocalRuleSet) (f : RuleFilter) :
  ...
  let simpTheoremsArrayNonempty : 0 < simpTheoremsArray.size := by
    simp [simpTheoremsArray'.snd, rs.simpTheoremsArrayNonempty, -List.length_pos]
  let rs := { rs with
